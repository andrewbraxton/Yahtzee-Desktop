...
void Engine::UpdateKeepFlag(int index, bool keep) { dice_[index].keep = keep; }
...
GameStates Engine::GetGameState() { return state_; }

int Engine::GetRollNumber() { return roll_number_; }
...
void Engine::Die::Roll() { value = ofRandom(1, 7); }
...
bool Engine::HasFullHouse(std::array<int, kMaxDieValue> counts) {
  return HasThreeOfAKind(counts) && std::any_of(counts.begin(), counts.end(),
                                                [](int i) { return i == 2; });
}
